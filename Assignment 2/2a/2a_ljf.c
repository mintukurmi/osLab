...
/*  this program is for: 2(a) Longest Time First (LJF)  */
#include <stdlib.h>
#include <stdio.h>
...
void LJF(Process *queue, int n, Process *schedule);
...
int main()
{
    ...
    printf("\n The final LFJ schedule: \n");
    LJF(process_queue, total_processes, process_schedule);
...
void LJF(Process queue[], int n, Process *schedule)
{
    ...
    while (executed != n)
    {
        ...
        for (i = 0; i < n; i++)
        {
            if (queue[i].burst_time > max_burst_time && queue[i].is_completed == 0)
            {
                idx = i;
                max_burst_time = queue[i].burst_time;
            }
        }
