...
/*  This program is  for: 2(a) preemptive Priority based scheduling algorithm    */
...
void premptive_priority(Process queue[], int n, Process *schedule);
...
int main()
{
    ...
    printf("\n The final Premptive Priority schedule: \n");
    premptive_priority(process_queue, total_processes, process_schedule);
...
void premptive_priority(Process queue[], int n, Process *schedule)
{
    ...
    while (executed != n)
    {
        ...
        for (i = 0; i < n; i++)
        { // finding the highest priority process with early arrival time
